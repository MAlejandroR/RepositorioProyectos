...
<nav
    aria-label="{{ __('filament::widgets/pagination.label') }}"
    role="navigation"
    {{
        $attributes->class([
...
>
    @if (! $paginator->onFirstPage())
        ...
        >
            {{ __('filament::widgets/pagination.actions.previous.label') }}
        </x-filament::button>
    @endif
    ...
    @if (! $isSimple)
        ...
        >
            {{
                trans_choice(
                    'filament::widgets/pagination.overview',
                    $paginator->total(),
                    [
    ...
    @if (count($pageOptions) > 1)
        <div class="col-start-2 justify-self-center">
            <label class="fi-pagination-records-per-page-select fi-compact">
                <x-filament::input.wrapper>
                    ...
                    >
                        @foreach ($pageOptions as $option)
                            <option value="{{ $option }}">
                                {{ $option === 'all' ? __('filament::widgets/pagination.fields.records_per_page.options.all') : $option }}
                            </option>
                        @endforeach
                    </x-filament::input.select>
                </x-filament::input.wrapper>
                ...
                <span class="sr-only">
                    {{ __('filament::widgets/pagination.fields.records_per_page.label') }}
                </span>
            </label>

            <label class="fi-pagination-records-per-page-select">
                <x-filament::input.wrapper
                    :prefix="__('filament::widgets/pagination.fields.records_per_page.label')"
                >
                    <x-filament::input.select
                        :wire:model.live="$currentPageOptionProperty"
                    >
    ...
    @if ($paginator->hasMorePages())
        ...
        >
            {{ __('filament::widgets/pagination.actions.next.label') }}
        </x-filament::button>
    @endif
    ...
    @if ((! $isSimple) && $paginator->hasPages())
        ...
        >
            @if (! $paginator->onFirstPage())
                @if ($extremeLinks)
                    <x-filament::pagination.item
                        :aria-label="__('filament::widgets/pagination.actions.first.label')"
                ...
                <x-filament::pagination.item
                    :aria-label="__('filament::widgets/pagination.actions.previous.label')"
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        <x-filament::pagination.item
                            ...
                            :aria-label="trans_choice('filament::widgets/pagination.actions.go_to_page.label', $page, ['page' => $page])"
                            :label="$page"
            ...
            @if ($paginator->hasMorePages())
                <x-filament::pagination.item
                    :aria-label="__('filament::widgets/pagination.actions.next.label')"
                ...
                @if ($extremeLinks)
                    <x-filament::pagination.item
                        :aria-label="__('filament::widgets/pagination.actions.last.label')"
